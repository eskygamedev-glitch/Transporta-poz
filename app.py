...
def fetch_data():
    ...
    for line in lines:
        a = line.split(",")
        a1 = a[2:4]
        print("Lat: {}, Lon: {}".format(a1[1], a1[0]))
        match = findMatch(float(a1[1])/1000000, float(a1[0])/1000000)
        
        if match != None:
            print("Match found: {}".format(match))

def findMatch(lat, lon):
    for stop in all_bus_stops:
        if (lat - delta) < stop[1] < (lat + delta) and (lon - delta) < stop[2] < (lon + delta):
            return stop[0]
    return None
